...
public class Parents {
    ...
    public void generateUsingTournament(GaVector population) {
        ...
        for (int i = 0; i < GA.SELECTION_FACTOR.get(); i++) {
            int randIdx = new Random().nextInt((int) GA.POPSIZE.get());
    ...
    public void generateUsingTournamentAge(GaVector population) {
        int bestFitness = Integer.MIN_VALUE;
        for (int i = 0; i < GA.SELECTION_FACTOR.get(); i++) {
            int randIdx = new Random().nextInt((int) GA.POPSIZE.get());
            if (population.geneVector.get(randIdx).age >= bestFitness) {
                indexP1 = randIdx;
                bestFitness = population.geneVector.get(randIdx).age;
            }
        }
